<h1 align="center"><br> TADS - Projeto de Redes de Computadores <br></h1>
...
- Quantidade de processadores / núcleos (0,4) :white_check_mark:
- Memória RAM livre (0,4) :white_check_mark:
- Espaço em disco livre (0,4) :white_check_mark:
- IPs das interfaces de rede, incluindo status (UP/DOWN) e tipo (loopback, ethernet, wifi) (0,4)
- Identificação do sistema operacional (0,4) :white_check_mark:
### 2.2 Servidor / Consolidação (2,0 pontos) :white_check_mark: 
- Dashboard em terminal ou interface gráfica simples com lista de clientes, última atualização,sistema operacional e IP principal (0,5) :white_check_mark:
...
- Funcionalidade de detalhamento de um cliente selecionado (0,5) :white_check_mark:
- Exportação de relatórios do consolidado geral e de um cliente específico nos formatos CSV ou JSON (0,5) :white_check_mark:

## 3. REQUISITOS PRINCIPAIS (4,0 PONTOS)
- Arquitetura Cliente/Servidor (1,0) :white_check_mark:
- Descoberta automática de clientes na LAN utilizando técnicas como broadcast, multicast ou
mensagens periódicas de hello (1,0) :white_check_mark:
- Uso de sockets puros (TCP e/ou UDP) para comunicação do protocolo desenvolvido (1,0) :white_check_mark:
- Utilização do paradigma de Orientação a Objetos, com organização clara e modular do código (1,0) :white_check_mark:
...
## 5. BÔNUS (ATÉ 2,0 PONTOS) :white_check_mark:
- Controle remoto do mouse do cliente (1,0) :white_check_mark:
- Controle remoto do teclado do cliente (1,0) :white_check_mark:
